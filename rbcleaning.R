...
clean <- function(year) {
  allweeks <- read_csv(paste0("data/rb/", year, "/raw/allweeks.csv"))
  allweeks <- allweeks %>% group_by(Player) %>%
    summarize(Pos = "RB", GP = sum(G), GPct = 1 - ifelse(year != 2022 & GP < 10, (10 - GP)*.03, ifelse(year == 2022 & GP < 10, (10 - GP)*.03, 0)), 
              ATT = sum(ATT), ATTpG = ATT / GP, TGT = sum(TGT), TGTpG = TGT / GP, WeighTOUCHpG = ATTpG + 2*TGTpG, 
              GoodBadPct = (sum(FPTS >= 15) - sum(FPTS <= 10)) / GP, GreatTerriblePct = (sum(FPTS >= 20) - sum(FPTS <= 5)) / GP,
              StdDevFPTS = sd(FPTS), MedianFPTS = median(FPTS), MeanFPTS = mean(FPTS), AdjMeanFPTS = MeanFPTS/(StdDevFPTS^(1/4)) * 1.6175,
...
RB2021 <- clean(2021)
RB2022 <- clean(2022)
allyears <- RB2022 %>% full_join(RB2021) %>% left_join(RB2020) %>% left_join(RB2019)
